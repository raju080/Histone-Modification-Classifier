...
class Model:
    def __init__(self, filters, kernel_size, pool_type, regularizer, activation_type, epochs, batch_size, model_file, log_file=None):
        ...
        # logging.basicConfig(filename=model_file+'log.log', level=print)
        if log_file == None:
    ...
    def parameterPrint(self):
        print(
            "\n=========================== Parameters ===================================", file=self.log_file)
        print("# of Filters: " + str(self.filters), file=self.log_file)
        print("Kernel size: " + str(self.kernel_size), file=self.log_file)
        print("Pool type: " + self.pool_type, file=self.log_file)
        print("regularizer: " + self.regularizer, file=self.log_file)
        print("activation_type: " + self.activation_type, file=self.log_file)
        print("epochs: " + str(self.epochs), file=self.log_file)
        print("batch_size: " + str(self.batch_size), file=self.log_file)
        print(
            "============================================================================\n", file=self.log_file)
    ...
    def create_LSTM_model(self, sequence_shape, alpha=100, beta=0.01, bkg_const=[0.25, 0.25, 0.25, 0.25]):
        ...
        reverse_input = keras.Input(
            shape=(sequence_shape[1], sequence_shape[2]), name='reverse')
        print("Forward shape----------------" + str(forward_input))
        print("Reverse shape----------------" + str(reverse_input))
        model = Sequential()
        n_features = 512
        n_neurons = 5
    ...
    def createSiameseNetwork(self, embedding_network, sequence_shape):
        ...
        # output_layer = Dense(2, kernel_initializer='normal', kernel_regularizer=regularizers.l2(0.001), activation='softmax')(merge_layer)
    ...
    def trainModel(self, model, processed_dict, seed=0, model_file=None):
        ...
        print("\n=========================== Before model.fit ===================================", file=self.log_file)
        # Early stopping
    ...
    def trainSiameseNetwork(self, model, processed_pair_dict, seed=0, model_file=None):
        ...
        print('forward1 shape: ' + str(forward1.shape), file=self.log_file)
        print('reverse1 shape: ' + str(reverse1.shape), file=self.log_file)
        print('forward2 shape: ' + str(forward2.shape), file=self.log_file)
        print('reverse2 shape: ' + str(reverse2.shape), file=self.log_file)
        ...
        print('siamese predictions:', file=self.log_file)
        print(predictions[:20], file=self.log_file)
        n, bins, patches = plt.hist(predictions, bins=50)
        plt.savefig('dist.png')
        mn, mx, mean = np.min(predictions), np.max(predictions), np.mean(predictions)
        print('predictions: min', mn, 'max', mx,'mean', mean, file=self.log_file)
        ...
        print('y_train :', file=self.log_file)
        print(y1_train[:50], file=self.log_file)
        print('predictions train:', file=self.log_file)
        print(predictions_train[:50], file=self.log_file)
        ...
        test_accuracy = true_pred/len(predictions_test)
        print('test accuracy: ', test_accuracy, file=self.log_file)
        # test_accuracy = history.history['val_accuracy']
        print("========================================================================\n", file=self.log_file)
    ...
    def cross_val(self, model_type, processed_dict, fold, seed):
        ...
        for train_indexes, test_indexes in kFold.split(forward_shuffle, readout_shuffle):
            ...
            print('train-set auc score is: ' + str(auc_score), file=self.log_file)
            ...
            print('test-set auc score is: ' + str(auc_score), file=self.log_file)
            test_auc_scores.append(auc_score)
            test_accs.append(true_pred/(true_pred+false_pred))

        # print('auc scores: ' + str(test_auc_scores), file=self.log_file)
        print("\n", file=self.log_file)
        print('Mean train auc_scores of 5-fold cv is ' +
              str(np.mean(train_auc_scores)), file=self.log_file)
        
        # print('accuracies: ', str(test_accs))
        print('Mean test auc_scores of 10-fold cv is ' +
              str(np.mean(test_auc_scores)), file=self.log_file)
